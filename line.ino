...
// put your code here to run for ever
  if (!(digitalRead(Ls))  && !(digitalRead(Rs)))         // if all white // s3, s2, s1 on white
     {  MotorControl(1,1); Serial.println("forward");   }
  else if (!(digitalRead(Ls))  && (digitalRead(Rs)))    // if Right on black // S1 out
     {  MotorControl(0,1); Serial.println("left"); }

  else if ((digitalRead(Ls)) && (!digitalRead(Rs)))    // if Left on black // s2 out
     {  MotorControl(1,0); Serial.println("right"); }
  else if (!(digitalRead(Ls))  && !(digitalRead(Rs)))         // if all white // s3, s2, s1 on white
